...
DB_PATH = os.getenv("DB_PATH", "/opt/nodeseek-rss/data/state.sqlite3")
EXCLUDE_CATEGORIES = [
    x.strip().lower()
    for x in os.getenv("EXCLUDE_CATEGORIES", "trade,交易").split(",")
    if x.strip()
]
...
USER_AGENT = os.getenv(
    "USER_AGENT",
    "Mozilla/5.0 NodeSeek-RSS-Telegram-Bot/1.0"
)
...
def contains_excluded_category(values: Iterable[str]) -> bool:
    joined = " ".join(values).lower()

    for term in EXCLUDE_CATEGORIES:
        if not term:
            continue

        if term == "交易" and "交易" in joined:
            return True
...
def is_excluded(entry: Any) -> bool:
    ...
    if contains_excluded_category(categories):
        return True

    # 兜底过滤：如果 RSS 把 trade 放在链接或 id 里，也会排除
    link_like = " ".join([
    ...
    for term in EXCLUDE_CATEGORIES:
        ...
        if re.search(rf"(^|[/?#&=._-]){re.escape(term)}($|[/?#&=._-])", link_like):
            return True

    # 再兜底：有些 RSS 可能会在摘要里写“分类：交易”或“版块：交易”
    summary = clean_text(entry.get("summary")).lower()

    if re.search(r"(分类|版块|category)\s*[:：]\s*(trade|交易)", summary, re.I):
        return True
...
def process_once(conn: sqlite3.Connection, seed_only: bool = False) -> None:
    ...
    for entry in entries:
        ...
        if is_excluded(entry):
            logging.info("跳过排除板块：%s", clean_text(entry.get("title")))
            mark_seen(conn, eid)
            continue
    ...
    if seed_only:
        ...
        logging.info("首次运行，已记录 %s 条现有帖子，不推送", len(pending))
        return
