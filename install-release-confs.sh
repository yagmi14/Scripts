...
# Default confdir for split configs
JSONS_PATH=${JSONS_PATH:-/etc/xray/confs}
...
install_xray() {
  ...
  # Install Xray configuration files to $JSONS_PATH (confdir)
# shellcheck disable=SC2153
JSONS_PATH=${JSONS_PATH:-/etc/xray/confs}
install -d "$JSONS_PATH"

if [[ ! -f "${JSONS_PATH}/00_log.json" ]]; then
cat >"${JSONS_PATH}/00_log.json" <<'EOF'
{
  "log": {
    "loglevel": "warning"
  }
...
if [[ ! -f "${JSONS_PATH}/10_in_vless_42001.json" ]]; then
cat >"${JSONS_PATH}/10_in_vless_42001.json" <<'EOF'
{
  "inbounds": [
    {
      "tag": "in-vless-42001",
      ...
      "settings": {
        "clients": [
          {
            "id": "6b0d9cd5-ef13-41ed-a084-43e3c65c3670",
            "flow": "xtls-rprx-vision"
          }
        ],
        "decryption": "mlkem768x25519plus.native.600s.GraQwya1Eooj1zlOoB3v77PxD6V7BBG9-cO415eOIS2sTTFHjT-UbfZ5JEpWSmUuikv29y1QdgdIeyHIJyj4Gw"
      }
    }
  ]
}
...
if [[ ! -f "${JSONS_PATH}/20_out_freedom.json" ]]; then
cat >"${JSONS_PATH}/20_out_freedom.json" <<'EOF'
{
  "outbounds": [
    {
      "tag": "direct",
