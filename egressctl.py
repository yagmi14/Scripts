...
def find_proxy(mihomo: Dict[str, Any], name: str) -> Optional[Dict[str, Any]]:
    return find_entry_by_name(mihomo.get("proxies") or [], name)


def find_ew_node(ew: Dict[str, Any], name: str) -> Optional[Dict[str, Any]]:
    return find_entry_by_name(ew.get("nodes") or [], name)
...
def listener_matches(listener: Dict[str, Any], name: str, port: Optional[int] = None) -> bool:
    ...
    if proxy_name == name or lname == name or lname == f"{name} / Socks":
        return True
    if port is not None:
        try:
...
def collect_node_views(mihomo: Dict[str, Any], ew: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for name in names:
        ew_node = find_ew_node(ew, name)
        ew_socks = norm_name(ew_node.get("socks")) if ew_node else ""
        ...
        views.append(
            {
                ...
                "socks": socks,
                "port": parse_port_from_url(socks) if socks else (int(listener.get("port")) if listener and listener.get("port") else None),
                "proxy": proxy,
                "listener": listener,
                "ew_node": ew_node,
            }
...
def print_groups(mihomo: Dict[str, Any], ew: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    for gi, group in enumerate(COUNTRY_GROUPS, start=1):
        ...
        for ni, node in enumerate(nodes, start=1):
            socks = f"  {node['socks']}" if node.get("socks") else "  未找到 socks 入站"
            ptype = as_dict(node.get("proxy")).get("type") or "no-proxy"
            print(f"    {ni}. {node['name']}  [{ptype}]{socks}")
    print("")
    return grouped
...
def remove_named_items(mihomo: Dict[str, Any], ew: Dict[str, Any], name: str, port: Optional[int] = None) -> None:
    if isinstance(mihomo.get("proxies"), list):
        mihomo["proxies"] = [p for p in mihomo["proxies"] if norm_name(as_dict(p).get("name")) != name]
    if isinstance(mihomo.get("listeners"), list):
        mihomo["listeners"] = [
            l for l in mihomo["listeners"] if not listener_matches(as_dict(l), name, port)
        ]
    if compatibility_nodes_look_like_egress(mihomo.get("nodes")):
        mihomo["nodes"] = [n for n in mihomo.get("nodes") or [] if norm_name(as_dict(n).get("name")) != name]
    if isinstance(ew.get("nodes"), list):
        ew["nodes"] = [n for n in ew["nodes"] if norm_name(as_dict(n).get("name")) != name]

    for group in mihomo.get("proxy-groups") or []:
        gd = as_dict(group)
        proxies = gd.get("proxies")
        if isinstance(proxies, list):
            gd["proxies"] = [p for p in proxies if p != name]
...
def rename_references(mihomo: Dict[str, Any], old_name: str, new_name: str) -> None:
    ...
    for group in mihomo.get("proxy-groups") or []:
        ...
        proxies = gd.get("proxies")
        if isinstance(proxies, list):
            gd["proxies"] = [new_name if p == old_name else p for p in proxies]
    for listener in mihomo.get("listeners") or []:
        ld = as_dict(listener)
        if norm_name(ld.get("proxy")) == old_name:
            ld["proxy"] = new_name
        if norm_name(ld.get("name")) in {old_name, f"{old_name} / Socks"}:
            ld["name"] = f"{new_name} / Socks"


def upsert_proxy(mihomo: Dict[str, Any], old_name: Optional[str], proxy: Dict[str, Any]) -> None:
    proxies = ensure_list(mihomo, "proxies")
    target_name = norm_name(proxy.get("name"))
    existing = None
    if old_name:
        existing = find_entry_by_name(proxies, old_name)
    if existing is None:
        existing = find_entry_by_name(proxies, target_name)
    if existing is None:
...
def upsert_listener(mihomo: Dict[str, Any], old_name: Optional[str], new_name: str, port: int) -> None:
    ...
    if existing is None:
        existing = find_listener(mihomo, new_name, port)
    if existing is None:
        existing = {}
...
def upsert_egress_node(ew: Dict[str, Any], old_name: Optional[str], new_name: str, socks_url: str) -> None:
    ...
    existing = None
    if old_name:
        existing = find_entry_by_name(nodes, old_name)
    if existing is None:
        existing = find_entry_by_name(nodes, new_name)
    if existing is None:
        nodes.append({"name": new_name, "socks": socks_url})
        return
...
def rebuild_managed_proxy_groups(mihomo: Dict[str, Any]) -> None:
    ...
    for proxy in proxies:
        name = norm_name(as_dict(proxy).get("name"))
        if not name:
            continue
...
def action_edit(mihomo: Dict[str, Any], ew: Dict[str, Any]) -> None:
    ...
    if uri:
        try:
            ...
            parsed_name = norm_name(parsed_proxy.get("name"))
            if parsed_name:
    ...
    new_name = ask("节点 name", old_name, required=True)
    port = int(old_port) if old_port else choose_free_port(mihomo, ew)
    socks_url = make_socks_url(port)

    if new_name != old_name and (find_ew_node(ew, new_name) or find_proxy(mihomo, new_name)):
        if not ask_bool(f"目标名称 {new_name} 已存在，是否覆盖", False):
            return
        remove_named_items(mihomo, ew, new_name, parse_port_from_url(norm_name(find_ew_node(ew, new_name).get("socks"))) if find_ew_node(ew, new_name) else None)

    if parsed_proxy is not None:
        parsed_proxy["name"] = new_name
        upsert_proxy(mihomo, old_name, parsed_proxy)
    else:
        existing_proxy = find_proxy(mihomo, old_name)
        if existing_proxy is not None:
            existing_proxy["name"] = new_name
            upsert_proxy(mihomo, old_name, existing_proxy)

    rename_references(mihomo, old_name, new_name)
    if parsed_proxy is not None or find_proxy(mihomo, new_name):
